...
type CourtFilterQuery = {
  branchId?: string;
  arenaId?: string | { $in: unknown[] };
};

type CourtPayload = CourtFilterQuery & {
  name: string;
  [key: string]: unknown;
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = (await request.json()) as CourtPayload;
